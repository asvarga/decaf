...
class HashTable {
    ...
    public static void main(String[] args) {
        ...
        n = new Integer(99);
        if (ht.contains(n))
            IO.putString("Table contains non-inserted element!");

        // this should be in the table
        if (!ht.contains(p)) {
            IO.putString("Table does not contain inserted element!");
            return;
        }
        ...
        ht.remove(p);
        if (ht.contains(p)) {
            IO.putString("Table contains removed element!");
            return;
        }

        // done
        IO.putString("Tests pass.");
    }
}
